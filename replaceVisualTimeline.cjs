...
import { Clock, Loader2, Play, CheckCircle, Sparkles, Coffee, CalendarClock } from 'lucide-react';
import { format, addMinutes, differenceInHours, startOfDay, endOfDay, setHours } from 'date-fns';
...
export default function VisualTimeline() {
  ...
  useEffect(() => {
    fetchTimeline();
    // Default available hours to remaining hours until 6 PM, min 1 hour, max 12 hours
    const now = new Date();
    const endOfWorkday = setHours(now, 18);
    let diff = differenceInHours(endOfWorkday, now);
    if (diff < 1) diff = 2; // Default to 2 if it's past 6 PM
    if (diff > 12) diff = 8;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.schedule) {
          ...
          toast.success("Timeline optimized for your real-time availability!");
  ...
  return (
    <div className="p-6 md:p-12 max-w-2xl mx-auto w-full">
      <header className="mb-10 pt-4 flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        <div className="flex-1">
          ...
          <div className="mt-4 flex items-center gap-3 bg-[#FBFDF8] border border-[#E0E3DB] p-3 rounded-2xl w-max">
            <CalendarClock className="w-5 h-5 text-[#3A693A]" />
            <div className="flex items-center gap-2">
               <span className="text-sm font-bold text-[#424940]">Time available today:</span>
               <select 
                  value={availableHours} 
                  onChange={(e) => setAvailableHours(Number(e.target.value))}
                  className="bg-transparent text-sm font-bold text-[#3A693A] border-none focus:ring-0 cursor-pointer outline-none"
               >
